...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Username`: `character varying`, `Firstname`: `character varying`, `Lastname`: `character varying`, `Email`: `character varying`, `Contact`: `character varying`, `CreatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
